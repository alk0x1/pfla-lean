...
instance : Repr Term where
  ...
  | lam x t, _ => Std.Format.bracket "(" (Std.Format.text ("lam " ++ x ++ " . ") ++ repr t) ")"
  | app t1 t2, _ => Std.Format.bracket "(" (repr t1 ++ Std.Format.text " " ++ repr t2) ")"
  | zero, _ => Std.Format.text "zero"
  | suc t, _ => Std.Format.bracket "(" (Std.Format.text "suc " ++ repr t) ")"
  | case t1 t2 x t3, _ => Std.Format.bracket "(" (Std.Format.text ("case " ++ repr t1 ++ " " ++ repr t2 ++ " " ++ x ++ " " ++ repr t3)) ")"
  | mu x t, _ => Std.Format.bracket "(" (Std.Format.text ("mu " ++ x ++ " . " ++ repr t)) ")"
...
| zero => zero
| (suc t) => suc (subst x v t)
| (case t1 t2 y t3) => case (subst x v t1) (subst x v t2) y (subst x v t3)
| (mu y t) => if x = y then mu y t else mu y (subst x v t)
