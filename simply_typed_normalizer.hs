...
synth ctx (App rator rand) = do
  -- if Γ |- e1 ⇒ A → B Γ |- e2 => A then Γ |- e1 e2 ⇒ B
  ty <- synth ctx rator
  case ty of
    TArr argT retT -> do
      check ctx rand argT
...
synth ctx (Rec ty tgt base step) = do
  -- if Γ |- n ⇒ Nat Γ |- b ⇐ A Γ |- s ⇐ Nat → A → A then Γ |- rec[A] n b s ⇒ A
  tgtT <- synth ctx tgt
  case tgtT of
    TNat -> do
      check ctx base tgtT
...
check ctx (Add1 n) t =
  -- if Γ |- e ⇒ t2 and t2 = t1 then Γ |- e ⇐ t1
  case t of
    TNat ->
      check ctx n TNat
    other ->
